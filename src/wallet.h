...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 MBR at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zMBR send
enum ZerocoinSpendStatus {
    ZMBR_SPEND_OKAY = 0,                            // No error
    ZMBR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMBR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMBR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMBR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMBR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMBR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMBR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMBR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMBR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMBR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMBR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMBR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMBR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMBR_SPENT_USED_ZMBR = 14,                      // Coin has already been spend
    ZMBR_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZMbrAutoBackups(bool fEnabled)
    {
    ...
    /** zMBR reset */
    boost::signals2::signal<void()> NotifyzMBRReset;
};
