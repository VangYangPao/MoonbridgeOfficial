...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MBR);
    ...
    if (!settings.contains("nAnonymizeMoonbridgeAmount"))
        settings.setValue("nAnonymizeMoonbridgeAmount", 1000);

    nAnonymizeMoonbridgeAmount = settings.value("nAnonymizeMoonbridgeAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMoonbridgeAmount"))
        SoftSetArg("-anonymizemoonbridgeamount", settings.value("nAnonymizeMoonbridgeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in moonbridge.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMoonbridgeAmount:
            return QVariant(nAnonymizeMoonbridgeAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMoonbridgeAmount:
            nAnonymizeMoonbridgeAmount = value.toInt();
            settings.setValue("nAnonymizeMoonbridgeAmount", nAnonymizeMoonbridgeAmount);
            emit anonymizeMoonbridgeAmountChanged(nAnonymizeMoonbridgeAmount);
            break;
        case CoinControlFeatures:
