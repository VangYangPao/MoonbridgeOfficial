...
#include "ui_zmbrcontroldialog.h"
...
std::list<std::string> ZMbrControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMbrControlDialog::listMints;

ZMbrControlDialog::ZMbrControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMbrControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMbrControlDialog::~ZMbrControlDialog()
{
    delete ui;
}

void ZMbrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMbrControlDialog::updateList()
{
...
void ZMbrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMbrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMbr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMbrControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZMbrControlDialog::GetSelectedMints()
{
...
void ZMbrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
